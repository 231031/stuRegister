...
import { getInfoTeacher, updateInfoTeacher } from "../../helpers/adminHelper";
...
export default function Adminupdateteacher() {
  ...
  useEffect(() => {
    const apiStu = async () => {
      try {
        const res = await getInfoTeacher(location.state.teacher_id);
        setTeacher(res);
      } catch (error) {
        console.error(error);
      }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const res = await updateInfoTeacher(values, location.state.teacher_id);
